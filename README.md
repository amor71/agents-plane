...
flowchart TD
    subgraph GW[Google Workspace]
        subgraph AC[Admin Console]
            Alice["Alice 🤖<br/>Agent: ON"]
            Bob["Bob 🤖<br/>Agent: ON"]
            Carol["Carol<br/>Agent: OFF"]
        end
    end

    subgraph AS[Apps Script Trigger]
        Poll["Polls every 5 min<br/>Detects schema changes"]
    end

    subgraph CF[Cloud Function]
        Provision["provision-agent<br/>Creates/stops VMs, manages secrets"]
    end

    subgraph GCP[Google Cloud Platform]
        subgraph VPC["VPC: agents-plane-vpc (no external IPs)"]
            VM1["agent-alice<br/>e2-standard-2<br/>OpenClaw"]
            VM2["agent-bob<br/>e2-standard-2<br/>OpenClaw"]
        end
        SM["Secret Manager<br/>agent-alice-config<br/>agent-bob-config"]
        VM1 --> SM
        VM2 --> SM
    end
    ...
    style GCP fill:#1a1a2e,stroke:#0f3460
    style VPC fill:#16213e,stroke:#0f3460
```

> **Access:** IAP Tunnel only (SSH via `gcloud compute ssh --tunnel-through-iap`)
...
| `script.scriptapp` | Execute Apps Script functions (trigger setup) |
   ```
